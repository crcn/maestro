...
module.exports = structr({
	...
	"step getImages": function(callback) {
		...
		this._ec2.call("DescribeImages", { "Owner.1": "self" }, function(err, result) {
			...
			images.forEach(function(image) {
				logger.info(sprintf("registering AIM %s", image.name));
	...
	"step getServerInfo": function(server, callback) {
		this._callIsetMethod(server, "DescribeInstances", callback);
	},
	...
	"_mapIsetCallback": function(callback) {
		...
		return outcome.error(callback).success(function(result) {
			return callback(null, self._mapIset(result.reservationSet || result));
		});
	}
});

...
exports.require = ["plugin-mongodb", "transport.core", "queryWatcher"];
	...
	Server.method({
		...
		start: function(callback) {
			...
			if(/stopping/.test(this.state)) {
				logger.info(sprintf("server is busy stopping, waiting... id=", self._id));
				setTimeout(function() {
					self.sync(function() {
						self.start(callback);
					});
		...
		destroy: function(callback) {
			...
			step(
				...
				//destroy first
				function() {
					getService(self).destroyServer(self, callback);
				},

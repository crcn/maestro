				...
				step(
					...
					on.success(function() {
						...
						if(/stopped/.test(state)) {
							// self._target.startServer(self, waitUntilReady);

							self._target.client.query("StartInstances", { InstanceId: self.get("_id") }, waitUntilReady, waitUntilReady);
						} else {
			...
			step(
				...
				on.success(function() {
					...
					if(/stopping/.test(state)) {
						...
						self._target.client.query("StopInstances", { InstanceId: self.get("_id") }, waitUntilStopped, waitUntilStopped);
					}
				})
			);
		},

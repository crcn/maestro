...
module.exports = structr({
	...
	"step exec": function(callback) {
		...
		step(
			...
			on.success(function(count) {
				...
				var count = collection.count(ops.query).sync();
			...
			function() {
				...
				this(null, collection.find(q).limit(limit).sort(ops.sort).sync());
			},
	...
	"step run": function(command, args, next) {
		this._execResultCommand("run", command, args, next);
	},
